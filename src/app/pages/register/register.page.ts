...
export class RegisterPage implements OnInit {
  login: AppUser = {restaurantCode: '',email: '',fullName: ''} as AppUser;
  submitted = false;
  restaurantName:String = '';
  isLogin: boolean = false;
roles: any =['waiter','chef','cashier','manager','supplier','delivery'];
  selected: any;
  ...
  constructor(private router: Router,
    ...
    ngOnInit() {
      this.user$.subscribe((userData: any) => {
        ...
        this.login.email = this.currentUser.email;
        this.login.fullName = this.currentUser.displayName;
        this.login.phone = this.currentUser.phone;
      ...
      this.route.queryParams.subscribe((data: any) => {
        ...
        if(data){
          this.login.restaurantCode = data.id;
  ...
  onLogin(form: NgForm): any {
    ...
    if (form.valid) {
      ...
      if (format.test(this.login.fullName)) {
      ...
      this.isLogin = true;
      this.login.createdAt = serverTimestamp();
      this.login.status = "ACTIVE";
      this.api.register(this.login).then((userData: any) => {
        console.log('userData:: ',userData);
        this.isLogin = false;

        const navData: NavigationExtras = {
          queryParams: {
            id: this.login.restaurantCode,
